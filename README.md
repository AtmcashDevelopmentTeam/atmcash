# Atmcash Reference Software (Atmcash Wallet)

HDD-mined cryptocurrency using an energy efficient
and fair Proof-of-Capacity (PoC) consensus algorithm (BURST/NXT based)

This wallet version is developed and maintained by the Atmcash Development Team (ADT). The two supported database servers are:
...
- 4 minute block time
- Total Supply: [2,158,812,800 ATM](https://atmwiki.org/en/block-reward/)
...
**NOTE: `atm.sh` is now deprecated and will not be included with the next release.**
...
Add this password to the `brs.properties` file you will create when installing ARS:
...
Please see the [Wiki article](https://atmwiki.org/en/testnet/) for details on how to setup a testnet node.
...
Your packaged release will now be available in `dist/atmcash-2.4.0.zip`
