...
public class IndirectIncomingServiceImplTest {
    ...
    @Test
    public void testIndirectIncomingServiceImplTestMultiOutTransaction() throws AtmException.NotValidException {
        ...
        recipients.add(Arrays.asList(1L, Constants.ONE_ATM));
        recipients.add(Arrays.asList(2L, Constants.ONE_ATM));
        recipients.add(Arrays.asList(3L, Constants.ONE_ATM));
        recipients.add(Arrays.asList(4L, Constants.ONE_ATM));
    ...
    @Test
    public void testIndirectIncomingServiceImplTestMultiOutSameTransaction() throws AtmException.NotValidException {
        addIndirectIncomingsRunnable.set(indirectIncomings -> {
            assertEquals(4, indirectIncomings.size());
            assertEquals(new HashSet<>(indirectIncomings).size(), indirectIncomings.size()); // Assert that there are no duplicates
        });
