...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.loadAsset = function(asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_atm").html($.t("sell_asset_for_atm", {
                "assetName": String(asset.name).escapeHTML()
            }));
            $("#buy_asset_with_atm").html($.t("buy_asset_with_atm", {
                "assetName": String(asset.name).escapeHTML()
            }));
        ...
        if (BRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_atm_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_atm_balance").html(BRS.formatAmount(BRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_order_modal").on("show.bs.modal", function(e) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "atm": BRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "atm": BRS.formatAmount(priceNQTPerWholeQNT, false, true),
                "total_atm": totalNXT
            });
        } else {
        ...
        $("#asset_order_fee_paid").html(BRS.formatAmount(feeNQT) + " ATM");
