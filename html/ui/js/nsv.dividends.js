var NSV = (function(NSV, $, undefined) {
    ...
    NSV.getAccountError = function(accountId, callback) {
        BRS.sendRequest("getAccount", {
            ...
            if (response.publicKey) {
                callback({
                    "type": "info",
                    "message": "The recipient account has a public key and a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " ATM.",
                    "account": response
                });
            }
            else {
                if (response.errorCode) {
                    ...
                    callback({
                        "type": "warning",
                        "message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " ATM. Please double check your recipient address before submitting.",
                        "account": response
                    });
                }
            }
    ...
    NSV.div_send_calc_dividends = function () {
        ...
        if (!(/^\d+$/.test(asset))) {
            ...
            BRS.sendOutsideRequest("/ATM?requestType=" + "getAsset", {
                "asset": asset
            }, function(response) {
                if (response.errorCode) {
                ...
                BRS.sendOutsideRequest("/ATM?requestType=" + "getTransaction", {"transaction": asset}, function(response) {
                    if (response.errorCode) {
                ...
                BRS.sendOutsideRequest("/ATM?requestType=" + "getTime", {}, function(response) {
                    if (response.errorCode) {
                ...
                if (list_nodist_acc !== "") {
                    if (list_nodist_acc !== "ISSUER") {
                        ...
                        for (i=0; i<account_array.length; ++i) {
                            var act_acc = account_array[i];
                            BRS.sendOutsideRequest("/ATM?requestType=" + "getAccount", {
                                "account": act_acc
                            }, function(response) {
                                if (response.errorCode) {
                ...
                BRS.sendOutsideRequest("/ATM?requestType=" + "getTrades", {
                    "asset": asset
                }, function(response) {
                    if (response.errorCode) {
                        ...
                        for (var i=0;i<trade_arr.length;i++) {
                            if (trade_arr[i].timestamp <= activate_timestamp) {
                                ...
                                BRS.sendOutsideRequest("/ATM?requestType=" + "getTransaction", {
    ...
    NSV.div_send_get_transactions = function(cur_asset, asset_array,initial_time,final_time) {
        ...
        for (var i=0; i<len; i++) {
            ...
            BRS.sendOutsideRequest("/ATM?requestType=" + "getAccountTransactionIds", {
