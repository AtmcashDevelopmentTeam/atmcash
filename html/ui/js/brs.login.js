...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.login = function(password, callback) {
        if (!password.length) {
            ...
            if ( BRS.hasLocalStorage ) {
                if ( BRS.settings.remember_passphrase ) {
                    localStorage.setItem("atm.passphrase", $("#login_password").val());
                }
                else {
                    localStorage.removeItem("atm.passphrase");
                }
            }
        }
        ...
        BRS.sendRequest("getBlockchainStatus", function(response) {
            ...
            if (password.trim().toUpperCase().substring(0, 6) === "ATM-" && password.length === 26) {
