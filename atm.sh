...
you can activate your MainNet config with "atm.sh switch MainNet" 
...
function upgrade_conf () {
    ...
    then
        ...
        BRS="${BRS//atm\.rebroadcastPeers/P2P.rebroadcastTo}"
        BRS="${BRS//atm\.connectWellKnownFirst/P2P.NumBootstrapConnections}"
        ...
        BRS="${BRS//atm\.rebroadcastAfter/P2P.rebroadcastTxAfter}"
        BRS="${BRS//atm\.rebroadcastEvery/P2P.rebroadcastTxEvery}"
        ...
        BRS="${BRS//atm\.mockMining/DEV.mockMining}"
        ...
        # GPU-related params
        BRS="${BRS//atm\.oclVerify/GPU.Acceleration}"
        BRS="${BRS//atm\.oclAuto/GPU.AutoDetect}"
        BRS="${BRS//atm\.oclPlatform/GPU.PlatformIdx}"
        BRS="${BRS//atm\.oclDevice/GPU.DeviceIdx}"
        BRS="${BRS//atm\.oclMemPercent/GPU.MemPercent}"
        BRS="${BRS//atm\.oclHashesPerEnqueue/GPU.HashesPerBatch}"
...
function create_brs_db {
    ...
    echo "[+] Creating atm wallet db ($P_DATA)..."
...
function exists_or_get {
    if [ -f $1 ]; then
        ...
        wget https://download.cryptoguru.org/atm/wallet/$1
    fi
}
...
if [[ $# -gt 0 ]] ; then
    case "$MY_CMD" in
        ...
        "compile")
            ...
            if hash mvn 2>/dev/null; then
                ...
                echo Nevertheless, now you can start the wallet with ./atm.sh
            else
            ...
            ;;
        "h2shell")
            java -cp atm.jar org.h2.tools.Shell
            ;;
        "import")
            ...
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                if [[ $MY_ARG == "mariadb" ]]; then
                    ...
                    if exists_or_get brs.h2.zip ; then
                        mkdir -p "$MY_DIR/atm_db"
                        rm -f atm_db/atm.trace.db
                        if unzip brs.h2.zip ; then
                            if mv atm.mv.db "$MY_DIR/atm_db"; then
    ...
    java $BRS_DEVSTART -cp atm.jar:conf brs.Atm
fi
