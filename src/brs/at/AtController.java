...
public abstract class AtController {
    ...
    private static final Logger debugLogger = Atm.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG) ? logger : NOPLogger.NOP_LOGGER;
    ...
    private static int runSteps(AtMachineState state) {
        ...
        AtMachineProcessor processor = new AtMachineProcessor(state, Atm.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
    ...
    private static void listCode(AtMachineState state, boolean disassembly, boolean determineJumps) {
        ...
        AtMachineProcessor machineProcessor = new AtMachineProcessor(state, Atm.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
        ...
        while (payload <= freePayload - costOfOneAT && keys.hasNext()) {
            ...
            if (atAccountBalance >= AtConstants.getInstance().stepFee(at.getCreationBlockHeight())
                ...
                try {
                    ...
                    if (!Atm.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_4, blockHeight)) {
                        totalAmount = amount;
                    } else {
                        totalAmount += amount;
                    }
    ...
    private static long makeTransactions(AT at) throws AtException {
        long totalAmount = 0;
        if (!Atm.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_4, at.getHeight())) {
