...
public abstract class EntitySqlTable<T> extends DerivedSqlTable implements EntityTable<T> {
  ...
  EntitySqlTable(String table, TableImpl<?> tableClass, AtmKey.Factory<T> dbKeyFactory, DerivedTableManager derivedTableManager) {
    this(table, tableClass, dbKeyFactory, false, derivedTableManager);
  }

  EntitySqlTable(String table, TableImpl<?> tableClass, AtmKey.Factory<T> dbKeyFactory, boolean multiversion, DerivedTableManager derivedTableManager) {
  ...
  public final void checkAvailable(int height) {
    if (multiversion && height < Atm.getBlockchainProcessor().getMinRollbackHeight()) {
  ...
  @Override
  public T get(AtmKey nxtKey, int height) {
    DbKey dbKey = (DbKey) nxtKey;
