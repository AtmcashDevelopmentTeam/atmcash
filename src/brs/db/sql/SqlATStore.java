...
public class SqlATStore implements ATStore {
  ...
  private final AtmKey.LongKeyFactory<brs.at.AT> atDbKeyFactory = new DbKey.LongKeyFactory<brs.at.AT>(AT.ID) {
      @Override
      public AtmKey newKey(brs.at.AT at) {
        return at.dbKey;
      }
    };
  private final VersionedEntityTable<brs.at.AT> atTable;

  private final AtmKey.LongKeyFactory<brs.at.AT.ATState> atStateDbKeyFactory = new DbKey.LongKeyFactory<brs.at.AT.ATState>(AT_STATE.AT_ID) {
      @Override
      public AtmKey newKey(brs.at.AT.ATState atState) {
  ...
  private void saveATState(DSLContext ctx, brs.at.AT.ATState atState) {
    ctx.mergeInto(AT_STATE, AT_STATE.AT_ID, AT_STATE.STATE, AT_STATE.PREV_HEIGHT, AT_STATE.NEXT_HEIGHT, AT_STATE.SLEEP_BETWEEN, AT_STATE.PREV_BALANCE, AT_STATE.FREEZE_WHEN_SAME_BALANCE, AT_STATE.MIN_ACTIVATE_AMOUNT, AT_STATE.HEIGHT, AT_STATE.LATEST)
            ...
            .values(atState.getATId(), brs.at.AT.compressState(atState.getState()), atState.getPrevHeight(), atState.getNextHeight(), atState.getSleepBetween(), atState.getPrevBalance(), atState.getFreezeWhenSameBalance(), atState.getMinActivationAmount(), Atm.getBlockchain().getHeight(), true)
            .execute();
  }
  ...
  private void saveAT(DSLContext ctx, brs.at.AT at) {
    ctx.insertInto(
      ...
      brs.at.AT.compressState(at.getApCodeBytes()), Atm.getBlockchain().getHeight()
    ).execute();
  }
  ...
  public List<Long> getOrderedATs() {
    return Db.useDSLContext(ctx -> {
      return ctx.selectFrom(
              ...
              AT_STATE.NEXT_HEIGHT.lessOrEqual(Atm.getBlockchain().getHeight() + 1)
      ).and(
              ACCOUNT.BALANCE.greaterOrEqual(
                      AtConstants.getInstance().stepFee(Atm.getBlockchain().getHeight())
                              * AtConstants.getInstance().apiStepMultiplier(Atm.getBlockchain().getHeight())
              )
      ).and(
  ...
  @Override
  public AtmKey.LongKeyFactory<brs.at.AT> getAtDbKeyFactory() {
  ...
  @Override
  public AtmKey.LongKeyFactory<brs.at.AT.ATState> getAtStateDbKeyFactory() {
