...
public class SqlAliasStore implements AliasStore {
  ...
  private static final DbKey.LongKeyFactory<Alias.Offer> offerDbKeyFactory = new DbKey.LongKeyFactory<Alias.Offer>(ALIAS_OFFER.ID) {
      @Override
      public AtmKey newKey(Alias.Offer offer) {
        return offer.dbKey;
      }
    };
  ...
  @Override
  public AtmKey.LongKeyFactory<Alias.Offer> getOfferDbKeyFactory() {
    return offerDbKeyFactory;
  }

  private static final AtmKey.LongKeyFactory<Alias> aliasDbKeyFactory = new DbKey.LongKeyFactory<Alias>(ALIAS.ID) {

      @Override
      public AtmKey newKey(Alias alias) {
        return alias.dbKey;
      }
    };

  @Override
  public AtmKey.LongKeyFactory<Alias> getAliasDbKeyFactory() {
  ...
  private void saveOffer(Alias.Offer offer) {
    Db.useDSLContext(ctx -> {
      ctx.insertInto(ALIAS_OFFER, ALIAS_OFFER.ID, ALIAS_OFFER.PRICE, ALIAS_OFFER.BUYER_ID, ALIAS_OFFER.HEIGHT)
              .values(offer.getId(), offer.getPriceNQT(), (offer.getBuyerId() == 0 ? null : offer.getBuyerId()), Atm.getBlockchain().getHeight())
              .execute();
    });
  }
  ...
  private void saveAlias(DSLContext ctx, Alias alias) {
    ctx.insertInto(ALIAS).
      ...
      set(ALIAS.HEIGHT, Atm.getBlockchain().getHeight()).execute();
  }
