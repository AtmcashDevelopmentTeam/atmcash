...
public class SqlBlockchainStore implements BlockchainStore {
  ...
  private final TransactionDb transactionDb = Atm.getDbs().getTransactionDb();
  private final BlockDb blockDb = Atm.getDbs().getBlockDb();
  ...
  public Collection<Block> getBlocks(int from, int to) {
    return Db.useDSLContext(ctx -> {
      int blockchainHeight = Atm.getBlockchain().getHeight();
      return
  ...
  public Collection<Transaction> getTransactions(Account account, int numberOfConfirmations, byte type, byte subtype, int blockTimestamp, int from, int to, boolean includeIndirectIncoming) {
    int height = numberOfConfirmations > 0 ? Atm.getBlockchain().getHeight() - numberOfConfirmations : Integer.MAX_VALUE;
    if (height < 0) {
      throw new IllegalArgumentException("Number of confirmations required " + numberOfConfirmations + " exceeds current blockchain height " + Atm.getBlockchain().getHeight());
    }
    return Db.useDSLContext(ctx -> {
