...
public final class Peers {
  ...
  public static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
    ...
    if (myAddress != null && myAddress.endsWith(":" + TESTNET_PEER_PORT) && !Atm.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    myPeerServerPort = propertyService.getInt(Props.P2P_PORT);
    if (myPeerServerPort == TESTNET_PEER_PORT && !Atm.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    useUpnp = propertyService.getBoolean(Props.P2P_UPNP);
    shareMyAddress = propertyService.getBoolean(Props.P2P_SHARE_MY_ADDRESS) && ! Atm.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    if (myAddress != null && ! myAddress.isEmpty()) {
      try {
        ...
        if (!Atm.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
          if (port >= 0) {
    ...
    json.addProperty("application",  Atm.APPLICATION);
    json.addProperty("version",      Atm.VERSION.toString());
    ...
    if(propertyService.getBoolean(P2P_ENABLE_TX_REBROADCAST)) {
      rebroadcastPeers = Collections
              .unmodifiableSet(new HashSet<>(propertyService.getStringList(Atm.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_REBROADCAST_TO : Props.P2P_REBROADCAST_TO)));
    } else {
      rebroadcastPeers = Collections.emptySet();
    }

    List<String> wellKnownPeersList = propertyService.getStringList(Atm.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_BOOTSTRAP_PEERS : Props.P2P_BOOTSTRAP_PEERS);
    ...
    if (wellKnownPeersList.isEmpty() || Atm.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
    ...
    usePeersDb       = propertyService.getBoolean(Props.P2P_USE_PEERS_DB) && ! Atm.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    threadPool.runBeforeStart(new Runnable() {
      ...
      public void run() {
        ...
        if (usePeersDb) {
          ...
          loadPeers(Atm.getDbs().getPeerDb().loadPeers());
        }
    ...
    if (! Atm.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
  ...
  private static class Init {
    ...
    static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
      ...
      if (Peers.shareMyAddress) {
            ...
            if (gateway != null) {
              ...
              try {
                ...
                if (gateway.getSpecificPortMappingEntry(port, "TCP", portMapping)) {
                  ...
                  if (gateway.addPortMapping(port, port, localAddress.getHostAddress(), "TCP", "atmcash")) {
        ...
        port = Atm.getPropertyService().getBoolean(Props.DEV_TESTNET) ? TESTNET_PEER_PORT : Peers.myPeerServerPort;
  ...
  private static final Runnable peerConnectingThread = new Runnable() {
    ...
    private void updateSavedPeers() {
      Set<String> oldPeers = new HashSet<>(Atm.getDbs().getPeerDb().loadPeers());
      ...
      try {
        Atm.getStores().beginTransaction();
        Atm.getDbs().getPeerDb().deletePeers(toDelete);
        currentPeers.removeAll(oldPeers);
        Atm.getDbs().getPeerDb().addPeers(currentPeers);
        Atm.getStores().commitTransaction();
      } catch (Exception e) {
        Atm.getStores().rollbackTransaction();
        throw e;
      } finally {
        Atm.getStores().endTransaction();
      }
    }
  ...
  static Peer addPeer(final String address, final String announcedAddress) {
    ...
    if (Atm.getPropertyService().getBoolean(Props.DEV_TESTNET) && peer.getPort() > 0 && peer.getPort() != TESTNET_PEER_PORT) {
  ...
  public static Peer getAnyPeer(Peer.State state) {
    if(!connectWellKnownFinished) {
      ...
      if (wellKnownConnected >= connectWellKnownFirst) {
        ...
        logger.info("You can open your Atm Wallet in your favorite browser with: http://127.0.0.1:8125 or http://localhost:8125");
      }
    }
