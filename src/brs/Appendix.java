...
public interface Appendix {
  ...
  abstract class AbstractAppendix implements Appendix {
    ...
    AbstractAppendix(int blockchainHeight) {
      this.version = (byte)(Atm.getFluxCapacitor().getValue(FluxValues.DIGITAL_GOODS_STORE, blockchainHeight) ? 1 : 0);
    }
    ...
    public abstract void validate(Transaction transaction) throws AtmException.ValidationException;
  ...
  class Message extends AbstractAppendix {
    ...
    public Message(ByteBuffer buffer, byte transactionVersion) throws AtmException.NotValidException {
      ...
      if (messageLength > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
        throw new AtmException.NotValidException("Invalid arbitrary message length: " + messageLength);
      }
    ...
    @Override
    public void validate(Transaction transaction) throws AtmException.ValidationException {
      if (this.isText && transaction.getVersion() == 0) {
        throw new AtmException.NotValidException("Text messages not yet enabled");
      }
      if (transaction.getVersion() == 0 && transaction.getAttachment() != Attachment.ARBITRARY_MESSAGE) {
        throw new AtmException.NotValidException("Message attachments not enabled for version 0 transactions");
      }
      if (messageBytes.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
        throw new AtmException.NotValidException("Invalid arbitrary message length: " + messageBytes.length);
      }
    }
  ...
  abstract class AbstractEncryptedMessage extends AbstractAppendix {
    ...
    private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws AtmException.NotValidException {
      super(buffer, transactionVersion);
    ...
    public void validate(Transaction transaction) throws AtmException.ValidationException {
      if (encryptedData.getData().length > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
        throw new AtmException.NotValidException("Max encrypted message length exceeded");
      }
      if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
        ...
        throw new AtmException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
      }
    }
  ...
  class EncryptedMessage extends AbstractEncryptedMessage {
    ...
    public EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws AtmException.ValidationException {
      super(buffer, transactionVersion);
    }
    ...
    public void validate(Transaction transaction) throws AtmException.ValidationException {
      ...
      if (! transaction.getType().hasRecipient()) {
        throw new AtmException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
      }
      if (transaction.getVersion() == 0) {
        throw new AtmException.NotValidException("Encrypted message attachments not enabled for version 0 transactions");
      }
    }
  ...
  class EncryptToSelfMessage extends AbstractEncryptedMessage {
    ...
    public  EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws AtmException.ValidationException {
      super(buffer, transactionVersion);
    }
    ...
    public void validate(Transaction transaction) throws AtmException.ValidationException {
      super.validate(transaction);
      if (transaction.getVersion() == 0) {
        throw new AtmException.NotValidException("Encrypt-to-self message attachments not enabled for version 0 transactions");
      }
    }
  ...
  class PublicKeyAnnouncement extends AbstractAppendix {
    ...
    public void validate(Transaction transaction) throws AtmException.ValidationException {
      if (! transaction.getType().hasRecipient()) {
        throw new AtmException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
      }
      if (publicKey.length != 32) {
        throw new AtmException.NotValidException("Invalid recipient public key length: " + Convert.toHexString(publicKey));
      }
      long recipientId = transaction.getRecipientId();
      if (Account.getId(this.publicKey) != recipientId) {
        throw new AtmException.NotValidException("Announced public key does not match recipient accountId");
      }
      if (transaction.getVersion() == 0) {
        throw new AtmException.NotValidException("Public key announcements not enabled for version 0 transactions");
      }
      ...
      if (recipientAccount != null && recipientAccount.getPublicKey() != null && ! Arrays.equals(publicKey, recipientAccount.getPublicKey())) {
        throw new AtmException.NotCurrentlyValidException("A different public key for this account has already been announced");
      }
    }
