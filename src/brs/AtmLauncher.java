...
public class AtmLauncher {
    public static void main(String[] args) {
        Logger logger = LoggerFactory.getLogger(AtmLauncher.class);
        ...
        if (canRunGui) {
            try {
                Class.forName("brs.AtmGUI")
                        .getDeclaredMethod("main", String[].class)
                        .invoke(null, (Object) args);
            } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
                logger.warn("Your build does not seem to include the AtmGUI extension or it cannot be run. Running as headless...");
                Atm.main(args);
            }
        } else {
            Atm.main(args);
