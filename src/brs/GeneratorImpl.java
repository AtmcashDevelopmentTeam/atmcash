...
public class GeneratorImpl implements Generator {
  ...
  private final AtmCrypto atmCrypto = AtmCrypto.getInstance();
  ...
  public byte[] calculateGenerationSignature(byte[] lastGenSig, long lastGenId) {
    return atmCrypto.calculateGenerationSignature(lastGenSig, lastGenId);
  }

  @Override
  public int calculateScoop(byte[] genSig, long height) {
    return atmCrypto.calculateScoop(genSig, height);
  }
  ...
  public BigInteger calculateHit(long accountId, long nonce, byte[] genSig, int scoop, int blockHeight) {
    return atmCrypto.calculateHit(accountId, nonce, genSig, scoop, getPocVersion(blockHeight));
  }

  @Override
  public BigInteger calculateHit(long accountId, long nonce, byte[] genSig, byte[] scoopData) {
    return atmCrypto.calculateHit(accountId, nonce, genSig, scoopData);
  }

  @Override
  public BigInteger calculateDeadline(long accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int blockHeight) {
    return atmCrypto.calculateDeadline(accountId, nonce, genSig, scoop, baseTarget, getPocVersion(blockHeight));
  }
