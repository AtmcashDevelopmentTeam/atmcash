...
public class AliasServiceImpl implements AliasService {
  ...
  private final AtmKey.LongKeyFactory<Alias> aliasDbKeyFactory;
  private final VersionedEntityTable<Offer> offerTable;
  private final AtmKey.LongKeyFactory<Offer> offerDbKeyFactory;
  ...
  public void addOrUpdateAlias(Transaction transaction, Attachment.MessagingAliasAssignment attachment) {
    ...
    if (alias == null) {
      AtmKey aliasDBId = aliasDbKeyFactory.newKey(transaction.getId());
  ...
  public void sellAlias(Transaction transaction, Attachment.MessagingAliasSell attachment) {
    ...
    if (priceNQT > 0) {
      ...
      if (offer == null) {
        AtmKey dbKey = offerDbKeyFactory.newKey(alias.getId());
