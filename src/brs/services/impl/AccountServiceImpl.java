...
import brs.db.AtmKey.LinkKeyFactory;
...
public class AccountServiceImpl implements AccountService {
  ...
  private final LongKeyFactory<Account> accountAtmKeyFactory;
  ...
  public AccountServiceImpl(AccountStore accountStore, AssetTransferStore assetTransferStore) {
    ...
    this.accountAtmKeyFactory = accountStore.getAccountKeyFactory();
  ...
  public Account getAccount(long id) {
    return id == 0 ? null : accountTable.get(accountAtmKeyFactory.newKey(id));
  }

  @Override
  public Account getAccount(long id, int height) {
    return id == 0 ? null : accountTable.get(accountAtmKeyFactory.newKey(id), height);
  }

  @Override
  public Account getAccount(byte[] publicKey) {
    final Account account = accountTable.get(accountAtmKeyFactory.newKey(getId(publicKey)));
  ...
  public Account getOrAddAccount(long id) {
    Account account = accountTable.get(accountAtmKeyFactory.newKey(id));
    if (account == null) {
  ...
  public void addToAssetBalanceQNT(Account account, long assetId, long quantityQNT) {
    ...
    AtmKey newKey = accountAssetKeyFactory.newKey(account.getId(), assetId);
  ...
  public void setRewardRecipientAssignment(Account account, Long recipient) {
    int currentHeight = Atm.getBlockchain().getLastBlock().getHeight();
    RewardRecipientAssignment assignment = getRewardRecipientAssignment(account.getId());
    if (assignment == null) {
      AtmKey atmKey = rewardRecipientAssignmentKeyFactory.newKey(account.getId());
      assignment = new RewardRecipientAssignment(account.getId(), account.getId(), recipient, (int) (currentHeight + Constants.ATM_REWARD_RECIPIENT_ASSIGNMENT_WAIT_TIME), atmKey);
    } else {
  ...
  public long getUnconfirmedAssetBalanceQNT(Account account, long assetId) {
    AtmKey newKey = Atm.getStores().getAccountStore().getAccountAssetKeyFactory().newKey(account.getId(), assetId);
